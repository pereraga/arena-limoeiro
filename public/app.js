  ...
  container.innerHTML = `
    <div class="max-w-7xl mx-auto px-4 py-6 sm:py-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm mb-6">
        ...
        <div class="grid grid-cols-2 md:flex md:w-auto items-center gap-2 w-full mt-3 md:mt-0">
          ${canDirectBooking() ? `
            <button onclick="openDirectBookingModal()" class="col-span-2 md:col-auto w-full md:w-auto justify-center px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs sm:text-sm font-black shadow-md flex items-center space-x-1.5 transition-all cursor-pointer">
              <i data-lucide="plus-circle" class="w-4 h-4 flex-shrink-0"></i>
              <span>⚡ Fazer Reserva Balcão</span>
            </button>
          ` : ''}

          <button onclick="syncDataFromSupabase().then(() => renderStepContent())" class="w-full md:w-auto px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center space-x-1.5 transition-all cursor-pointer shadow-2xs" title="Atualizar dados do banco">
            <i data-lucide="refresh-cw" class="w-4 h-4 text-emerald-600 flex-shrink-0"></i>
            <span>Atualizar</span>
          </button>

          <button onclick="logoutAdmin()" class="w-full md:w-auto px-3.5 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl text-xs sm:text-sm font-bold flex items-center justify-center space-x-1.5 transition-all cursor-pointer shadow-2xs">
